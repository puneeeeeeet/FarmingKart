...
export default function Herbicides() {
 ...
 const herbicideFilter = productData.filter((product)=>product.category=="Herbicide");
 
  const [showModal, setShowModal] = useState(false);

  return (
    <div className=" mt-3  grid grid-cols-2 sm:grid-cols-7 gap-2 ">
      {herbicideFilter.map((pro) => (
        <div
          key={pro.id}
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center">
            <p className="sm:text-xl">{pro.name}</p>
      ...
      {showModal ? (
        ...
        >
          <div className="w-64 h-32 rounded-md bg-gray-200 flex flex-col items-center justify-center">
            ...
            <Link
              href={"https://play.google.com/store/apps/details?id=com.scompany.kisaan24&hl=en_IN"}
